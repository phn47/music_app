...
// Thêm provider này vào file
final recentlyPlayedSongsProvider =
    StateNotifierProvider<RecentSongsNotifier, List<SongModel>>((ref) {
  return RecentSongsNotifier(ref.read(homeLocalRepositoryProvider));
});

class RecentSongsNotifier extends StateNotifier<List<SongModel>> {
  final HomeLocalRepository _repository;

  RecentSongsNotifier(this._repository) : super([]) {
    _loadInitialSongs();
  }

  void _loadInitialSongs() {
    state = _repository.loadSongs();
  }

  Future<void> addSong(SongModel song) async {
    await _repository.uploadLocalSong(song.toMap());
    _loadInitialSongs();
  }
}
